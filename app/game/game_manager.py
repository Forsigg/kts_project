...
class GameManager:
    STATES = {1: "started", 2: "question", 3: "waiting", 4: "checking", 5: "done"}
    ...
    async def init_from_game(self, game: Game) -> None:
        ...
        self.answers = await self.app.store.quizzes.get_answers_by_question_id(
            self.question.id
        )
    ...
    async def start_game(self, chat_id: int) -> None:
        ...
        self.scores = await self.app.store.games.create_start_scores(
            game_id=self.game.id, users_id=[user.id for user in self.users]
        )
        ...
        self.game = await self.app.store.games.update_state_in_game(
            game_id=self.game.id, state_id=2
        )
        self.state = 2

    async def send_question(self) -> None:
        await self.app.store.vk_api.send_group_message(
            Message(receiver_id=self.game.chat_id, text=f"{self.question.title}")
        )
        self.game = await self.app.store.games.update_state_in_game(
            game_id=self.game.id, state_id=3
        )
        self.state = 3
    ...
    async def user_kick(self, user_id: int) -> None:
        ...
        await self.app.store.vk_api.send_group_message(
            Message(
                receiver_id=self.chat_id, text=f"Игрок {lose_user.full_name} выбывает!"
            )
        )
    ...
    async def add_point(self, user_id: int) -> None:
        ...
        await self.app.store.games.update_total_in_score(
            score_id=score_obj.id, total=score_obj.total
        )
    ...
    async def check_answer(self, answer: Answer, user_id: int) -> None:
        if await self.is_correct_answer(answer):
            if answer.title in self.used_answers:
                await self.app.store.vk_api.send_group_message(
                    Message(receiver_id=self.chat_id, text=f"Такой ответ уже был.")
                )
            else:
    ...
    async def end_game(self, chat_id: int):
        self.game = await self.app.store.games.update_state_in_game(
            game_id=self.game.id, state_id=5
        )
        self.state = 5
        results = ""
        for score in self.scores:
        ...
        await self.app.store.vk_api.send_group_message(
            Message(receiver_id=chat_id, text=results)
        )

        await self.app.store.vk_api.send_group_message(
            Message(receiver_id=chat_id, text=f"Игра закончена, всем до свидания!")
        )
